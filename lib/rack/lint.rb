  ...
  class Lint
    ...
    class Wrapper
      ...
      def to_a
        ...
        assert("response is not an Array, but #{@response.class}") {
          @response.kind_of? Array
        }
        assert("response array has #{@response.size} elements instead of 3") {
          @response.size == 3
        }
      ...
      def finished(response)
        ...
        assert("Body has not been closed") { @closed }
      end
      ...
      def each
        ...
        assert("Chunked Body must respond to each") do
          @body.respond_to?(:each)
        end

        ## It must only be called once.
        assert("Response body must only be called once (#{@invoked})") do
          @invoked.nil?
        end
        ...
        @body.each do |chunk|
          ...
          assert("Body yielded non-string value #{chunk.inspect}") do
            chunk.kind_of?(String)
          end
          @size += chunk.bytesize
          yield chunk
        end
      ...
      def call(stream)
        ...
        ## It must only be called once.
        assert("Response body must only be called enumerated once (#{@invoked})") do
          @invoked.nil?
        end
      ...
      class StreamWrapper
        ...
        def initialize(stream)
          ...
          REQUIRED_METHODS.each do |method_name|
            assert("stream must respond to #{method_name}") do
              stream.respond_to?(method_name)
            end
          end
        end
      end
